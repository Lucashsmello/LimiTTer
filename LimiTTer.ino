...
//  pinMode(MOSIPin, OUTPUT); //not needed, it is done in SPI.begin().
...
void Inventory_Command() {
  ...
  byte error_code=sendCommandAndWait(0x04, // Send Receive CR95HF command
              0x26, // request Flags byte - ISO/IEC 15693
              0x01, // Inventory Command for ISO/IEC 15693
              0x00,// mask length for inventory command
              2000);
  if(error_code!=0){
    Serial.println("Error in waiting Inventory_Command response");
    NFCReady = 1;
    return;
  }
